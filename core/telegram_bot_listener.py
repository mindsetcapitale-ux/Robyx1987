...
def run_background_task(task_name, function):
    if RUNNING_TASKS.get(task_name):
        send_telegram_message(f"⏳ {task_name} già in esecuzione.")
        return
...
def get_market_data(symbol_name):
    ...
    try:
        ...
        print(f"Coinbase errore {symbol_name}:", e)
        return None
...
def send_gems():
    ...
    if not candidates:
        send_telegram_message("📭 Nessuna early gem trovata.")
        return
...
def send_momentum():
    ...
    if not results:
        send_telegram_message("📭 Nessun momentum forte.")
...
"""
        ...
        if active:
            ...
            send_telegram_message("🟢 Nessun task attivo.")
